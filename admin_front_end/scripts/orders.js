document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCurrentPage() {
        ...
        currentPageOrders.forEach(order => {
            ...
            Promise.all([
                fetch(`http://0.0.0.0:5000/api/v1/user/${order.user_id}`).then(response => response.json()),
                fetch(`http://0.0.0.0:5000/api/v1/medication/${order.medication_id}`).then(response => response.json()),
                fetch(`http://0.0.0.0:5000/api/v1/doctor/${order.doctor_id}`).then(response => response.json())
...
/*document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageSize = 10; // Number of items to display per page
    let currentPage = 1;
    let orderData = []; // Array to store all counties data
